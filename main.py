...
def main():
    ...
    if cfg.MODE == 'train':
        ...
        #     os.path.join(out_dir, '6')))
        # pr.dump_stats(os.path.join(out_dir, 'tp_train.dmp')) #测量程序中各部分代码的执行时间和调用次数，帮助找出程序中的瓶颈和优化空间,打印性能统计数据或将数据保存到文件中
    elif cfg.MODE == 'eval':
        ...
        if eval_mode == 'segment':
            ...
            # pr = cProfile.Profile()
            # pr.enable()
            global Confusion_Memory_Block
            Confusion_Memory_Block = Confusion_Memory_Block()
            
            eval_dict, eval_tools, eval_tool_upperbound = model_trainer.evaluate_inst(
                dataset_seg, dataset_inst, Confusion_Memory_Block, topk=cfg.eval.topK)
            # pr.disable()
            ...
            # TODO: test progress
            # Confusion_Memory_Block.update(front_box_list, back_box_list, relation_list, mode = 'Eval')
            # Confusion_Memory_Block.save_features(mode='test')
            # Confusion_Memory_Block.compute_IDM_BND() # read test and val dataset
