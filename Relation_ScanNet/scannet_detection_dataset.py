...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000, use_color=False, use_height=False, augment=False):

        self.data_path = os.path.join(BASE_DIR, 'scannet_train_detection_data_25classes')
        ...
        if split_set=='all':            
            ...
            for scan_name in self.scan_names:
                if scan_name.split('_')[-1] == '00' and int(scan_name.split('e')[-1].split('_')[0]) < K:
                    self.scan_names_00.append(scan_name)
            print('kept {} scans out of {}'.format(len(self.scan_names_00), num_scans))
        else:
    ...
    def __getitem__(self, idx):
        ...
        scan_name = self.scan_names_00[idx]
        ...
        sem_rel = np.load(os.path.join(self.data_path, scan_name)+'_sem_rel.npy')
        geo_rel = np.load(os.path.join(self.data_path, scan_name) + '_geo_rel.npy')
        same_plane_rel = np.load(os.path.join(self.data_path, scan_name) + '_same_plane_rel.npy')
        ...
        # ------------------------------- LABELS ------------------------------
        sem_rel_label = np.zeros((MAX_NUM_OBJ, MAX_NUM_OBJ))
        geo_rel_label = np.zeros((MAX_NUM_OBJ, MAX_NUM_OBJ))
        same_plane_rel_label = np.zeros((MAX_NUM_OBJ, MAX_NUM_OBJ))
        ...
        sem_rel_label[0:sem_rel.shape[0], 0:sem_rel.shape[1]] = sem_rel
        geo_rel_label[0:geo_rel.shape[0], 0:geo_rel.shape[1]] = geo_rel
        same_plane_rel_label[0:same_plane_rel.shape[0], 0:same_plane_rel.shape[1]] = same_plane_rel
        ...
        size_residuals[0:instance_bboxes.shape[0], :] = \
            target_bboxes[0:instance_bboxes.shape[0], 3:6] - DC.mean_size_arr[class_ind,:3]
            
        ret_dict = {}
        ret_dict['point_clouds'] = point_cloud.astype(np.float32)
        ret_dict['targets'] = target_bboxes.astype(np.float32)
        ...
        ret_dict['scan_idx'] = np.array(int(scan_name.split('e')[-1].split('_')[0])).astype(np.int64)  # np.array(idx).astype(np.int64)
        # ret_dict['pcl_color'] = pcl_color
        ret_dict['sem_rel_label'] = sem_rel_label.astype(np.int64)
        ret_dict['geo_rel_label'] = geo_rel_label.astype(np.int64)
        ret_dict['same_plane_rel_label'] = same_plane_rel_label.astype(np.int64)
        return ret_dict
